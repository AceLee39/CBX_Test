...
public class FormEntityExporterHQTest
{
  private static boolean initSprintSecurity = false;

  private static final Log log = LogFactory.getLog(FormEntityExporterHQTest.class);
    ...
    throws ActionException
  {
    ...
    final ImportRawData ctx = new ImportRawData(file, "EXCEL", file.getName());
    try {
      ActionDispatcher.execute(ctx);
    } catch (final Exception e) {
      log.info("ImportRawData result for:" + file.getName() + ">>>" + ctx.getLogMessage(), e);
    }
    log.info("ImportRawData result for:" + file.getName() + ">>>" + ctx.getResultValue(""));
  }
}
